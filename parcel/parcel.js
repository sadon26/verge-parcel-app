...
router.get("/", async (req, res, next) => {
    ...
    pool.query("SELECT * FROM Users WHERE role_id = $1", [id], (err, results) => {
      if (err) {
        throw err
      }
...
router.get("/:id", async (req, res, next) => {
  ...
  pool.query("SELECT * FROM Parcel WHERE user_id = $1", [id], (err, results) => {
    if (results.rowCount == 0) {
      res.status(400).json({
...
router.put('/destination/change/:id', (req, res, next) => {
    const {
      id
    } = req.body
  ...
  (req, res) => {
    ...
    pool.query("UPDATE Parcel SET destination = $1 WHERE user_id = $2", [destination, id], (err, results) => {
      if (err) {
        throw err
      }
      res.status(200).json({
...
router.delete('/cancel/:id', (req, res, next) => {
  ...
  (req, res) => {
    ...
    pool.query("DELETE FROM Parcel WHERE user_id = $1", [id], (err, results) => {
      if (err) {
        throw err
      }
      res.status(200).json({
...
router.put('/status/change/:id', async (req, res, next) => {
  const {
    id
  } = req.body
  pool.query("SELECT * FROM Roles WHERE id = $1", [id], (err, results) => {
  ...
  pool.query("UPDATE Parcel SET status = $1 WHERE user_id = $2", [status, id], (err, results) => {
    if (err) {
      throw err
    }
    res.json({
...
router.put('/location/change/:id', async (req, res, next) => {
  const {
    id
  } = req.body
  pool.query("SELECT * FROM Roles WHERE id = $1", [id], (err, results) => {
  ...
  pool.query("UPDATE Parcel SET location = $1 WHERE user_id = $2", [location, id], (err, results) => {
    if (err) {
      throw err
    }
    res.json({
